...
class SupabaseAuthService {
  ...
  Future<User> signUpUser({
    ...
    try {
      ...
      log("Exception in SupabaseAuthService.signUpUser: ${e.toString()} and ${e
          .message}");
      if (e.message.contains('Password should be at least 6 characters.')) {
        throw CustomException(
            message: 'The password provided is too weak. Should be at least 6 characters ');
  ...
  Future<User> signInUser({
    ...
    try {
      final AuthResponse response = await AuthRepo.supabase.auth
          .signInWithPassword(
        email: email,
        password: password,
      );
    ...
    on AuthException catch (e) {
      log("Exception in SupabaseAuthService.signInUser: ${e.toString()} and ${e
          .message}");
