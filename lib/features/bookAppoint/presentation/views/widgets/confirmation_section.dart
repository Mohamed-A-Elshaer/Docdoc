...
class ConfirmationSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
             Text('Booking Information',style: TextStyles.semiBold16.copyWith(color: const Color(0xff242424)),),
             const SizedBox(height: 14,),
             const BookingInfoItem(isBookingDetailsSection: false,),
             ...
             Text('Payment Information',style: TextStyles.semiBold16.copyWith(color: const Color(0xff242424)),),
             const SizedBox(height: 20,),
             Consumer<BookingEntity>(
               builder: (context, bookingEntity, child) {
                 final paymentOption = bookingEntity.paymentOptionEntity?.paymentOption ?? 'Not selected';
                 ...
                 return Row(
                   ...
                   children: [
                     Row(
                       children: [
                         Image.asset(Assets.imagesPaypal, height: 40, width: 40,),
                         const SizedBox(width: 8,),
                         Column(
                           crossAxisAlignment: CrossAxisAlignment.start,
                           children: [
                             Text(paymentOption, style: TextStyles.medium14.copyWith(color: const Color(0xff242424)),),
                             Text('***** ***** ***** 37842', style: TextStyles.regular10.copyWith(color: const Color(0xff616161)),),
                           ],
                         ),
                       ],
                     ),
                     const SizedBox(
                       width: 80,
                       height: 38,
                       child: CustomTextButton(text: 'Change')
                     )
                   ],
                 );
               },
 ...
 static void confirmationSheet(
    ...
    // Get BookingEntity from the original context before showing the modal
        ...
        return Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const SizedBox(height: 40,),
                Text('Payment Info',style: TextStyles.semiBold14.copyWith(color: const Color(0xff242424)),),
                const SizedBox(height: 32,),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Subtotal',style: TextStyles.regular14.copyWith(color: const Color(0xff757575)),),
                    Text('\$${doctorModel.appointPrice.toString()}',style: TextStyles.semiBold14.copyWith(color: const Color(0xff242424)),),
                  ],
                ),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Tax',style: TextStyles.regular14.copyWith(color: const Color(0xff757575)),),
                    Text('\$250',style: TextStyles.semiBold14.copyWith(color: const Color(0xff242424)),),
                  ],
                ),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Payment Total',style: TextStyles.semiBold16.copyWith(color: const Color(0xff242424)),),
                    Text('\$${(doctorModel.appointPrice+250).toString()}',style: TextStyles.semiBold16.copyWith(color: const Color(0xff242424)),),
                  ],
                ),
                    ...
                    builder: (context, bookingEntity, child) {
                      ...
                      return StatefulBuilder(
                        builder: (context, setLocalState) {
                          ...
                          // Get date and time from bookingEntity
                          final dateTimeEntity = bookingEntity.dateTimeEntity;
                          if (dateTimeEntity == null || dateTimeEntity.isAvailableTimeChosen != true) {
                            buildErrorBar(context, 'Please select date and time');
                            setLocalState(() => isBooking = false);
                            return;
                          }

                          // Parse the date string (format: "Wednesday, 08 May 2023")
                          ...
                          // Convert time from "02.00 PM" to "14:00" format
                          String time24Hour = _convertTo24HourFormat(dateTimeEntity.time);

                          // Format as "2026-01-15 14:00"
                          final dateTimeString = '${DateFormat('yyyy-MM-dd').format(selectedDate)} $time24Hour';

                          try {
                            // Book the appointment and get the raw response
                            ...
                            // Check if the response indicates success (message "Created Successfully" or "Created Successfuly", or we have valid data)
                            ...
                            if (response.containsKey('data')) {
                              ...
                              if (data is Map<String, dynamic>) {
                                // Data might be the appointment itself or contain an 'appointment' key
                                if (data.containsKey('id') && data.containsKey('doctor')) {
                                  appointmentData = data;
                                } else if (data.containsKey('appointment') && data['appointment'] is Map) {
                                  appointmentData = Map<String, dynamic>.from(data['appointment'] as Map);
                                } else {
                                  appointmentData = data;
                                }
                              ...
                              // Appointment data might be at root level
                              appointmentData = response;
                            }

                            // Success: explicit success message or valid appointment data (API may return 201 with only data)
                            final isSuccess = hasSuccessMessage || (appointmentData != null && appointmentData.containsKey('id') && appointmentData.containsKey('doctor'));

                            if (isSuccess && appointmentData != null) {
                              final appointment = AppointmentModel.fromJson(appointmentData);

                              // Save to Supabase 'appointments' table (user_uid, api_*, start_time, end_time, notes, appoint_price)
                              final userUid = Supabase.instance.client.auth.currentUser?.id;
                              if (userUid == null || userUid.isEmpty) {
                                buildErrorBar(context, 'You must be signed in to save the appointment.');
                                setLocalState(() => isBooking = false);
                                return;
                              }
                              ...
                              try {
                                ...
                                buildErrorBar(context, 'Appointment booked but could not save locally. Please try again later.');
                                setLocalState(() => isBooking = false);
                                return;
                              }
                              ...
                              // Call the success callback with the appointment model
                              ...
                              if (appointmentData == null) {
                                print('Appointment response has no data or unexpected structure. Keys: ${response.keys.toList()}');
                              }
  ...
  static String _convertTo24HourFormat(String time12Hour) {
    try {
      ...
      try {
        ...
        int minute = int.parse(parts[1].substring(0, 2)); // Get first 2 digits (minute)
